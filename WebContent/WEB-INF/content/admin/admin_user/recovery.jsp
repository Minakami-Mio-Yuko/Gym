	...
	<div class="row">
		<div class="col-md-2 text-center center-block">
			<ul class="verticalnav">
				...
				<li><a href="../admin_user/recovery"class="current">恢复账号</a></li>
		...
		<div class="col-md-10  center-block"><br/>
            <h3 style="color:white;" class="text-center"><%=s.getAttribute("messageUserRecovery")%></h3>
			<table class="table table-bordered table-hover">
				<tr id="userTitle">
					<td colspan="3" style="font-weight: bold">已注册且暂不可登录的账号</td>
				</tr>
				<tr id="userTitle">
					<td>操作</td>
					<td>账号</td>
			...
			<%
			for(int i=0;i<Integer.valueOf(s.getAttribute("userCountR").toString());i++){
			%>
				<form action="userRecovery" class="form-horizontal" method="post">
					<div class="form-group">
						<Tr id="userTitle">
						    <td><button type="submit" class="btn btn-success btn-block">恢复</button></td>						  
							<td><input type="text" class="form-control" name="userCode"
								readonly value="<%=s.getAttribute("userAll"+i+"codeR")%>"></td>
							<td><input type="text" class="form-control" name="userManager"
								readonly value="<%=s.getAttribute("userAll"+i+"roleR")%>"></td>
						</tr>
					</div>
				</form>
			<%
